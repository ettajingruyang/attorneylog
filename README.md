...
跑这个系统**不需要云服务器**，只需要一台能一直开着的电脑——办公室公用机、合伙人自己的电脑、甚至一台旧笔记本都行：

| 方式 | 花费 | 适合 |
|---|---|---|
| **① 办公室一台电脑** | 免费 | 团队基本在同一办公室：装好 Node.js，双击启动，同事浏览器打开 `http://电脑IP:7100` |
| **② 免费云平台一键部署**（Render / Railway 等） | 免费 | 团队分散各地、想随时访问：用 GitHub 账号授权后点几下即可（国内访问速度一般） |
| **③ 轻量云服务器**（阿里云/腾讯云轻量，¥30–100/月） | 低 | 最稳的选择：数据在自己手里；装 Docker 后一行 `docker compose up -d` |

### ① 办公室电脑版（最简单，推荐先试这个）

1. 到 [Releases](../../releases) 下载 **portable zip**（已构建好，解压即用，免编译）
2. 装一次 [Node.js](https://nodejs.org/)（免费，一直「下一步」就行）
3. 双击 `start.command`（Mac）或 `start.bat`（Windows）→ 浏览器自动打开
4. 团队同事在同一 Wi-Fi 下，浏览器输入 `http://这台电脑的IP:7100` 即可
5. 数据就是一个 JSON 文件，存在这个文件夹里——备份=拷贝，搬家=整个文件夹拖走

> 也可以不下载 Releases，直接把本仓库发给你的 AI 助手说"帮我在这台电脑上跑起来"，它会替你完成所有步骤。
